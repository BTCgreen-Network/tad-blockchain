...
from tad.data_layer.data_layer_util import NodeType, Status
from tad.data_layer.data_store import DataStore
from tad.types.blockchain_format.tree_hash import bytes32
...
def tad_root_fixture(tmp_path: pathlib.Path, scripts_path: pathlib.Path) -> TadRoot:
...
def closing_tad_root_popen(tad_root: TadRoot, args: List[str]) -> Iterator[None]:
    environment = {**os.environ, "TAD_ROOT": os.fspath(tad_root.path)}
...
@pytest.fixture(name="tad_daemon", scope="function")
def tad_daemon_fixture(tad_root: TadRoot) -> Iterator[None]:
    with closing_tad_root_popen(tad_root=tad_root, args=[sys.executable, "-m", "tad.daemon.server"]):
...
@pytest.fixture(name="tad_data", scope="function")
def tad_data_fixture(tad_root: TadRoot, tad_daemon: None, scripts_path: pathlib.Path) -> Iterator[None]:
    with closing_tad_root_popen(tad_root=tad_root, args=[os.fspath(scripts_path.joinpath("tad_data_layer"))]):
        # TODO: this is not pretty as a hard coded time
        # let it settle
        time.sleep(5)
        yield
