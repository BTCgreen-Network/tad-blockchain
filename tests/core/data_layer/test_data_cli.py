...
@pytest.mark.asyncio
async def test_help(tad_root: TadRoot) -> None:
    """Just a trivial test to make sure the subprocessing is at least working and the
    data executable does run.
    """
    completed_process = tad_root.run(args=["data", "--help"])
    assert "Show this message and exit" in completed_process.stdout


@pytest.mark.xfail(strict=True)
@pytest.mark.asyncio
def test_round_trip(tad_root: TadRoot, tad_daemon: None, tad_data: None) -> None:
    """Create a table, insert a row, get the row by its hash."""

    with tad_root.print_log_after():
        create = tad_root.run(args=["data", "create_data_store"])
        ...
        completed_process = tad_root.run(args=["data", "get_value", "--id", tree_id, "--key", key])
        parsed = json.loads(completed_process.stdout)
        expected = {"value": value, "success": True}
        assert parsed == expected
        get_keys_values = tad_root.run(args=["data", "get_keys_values", "--id", tree_id])
