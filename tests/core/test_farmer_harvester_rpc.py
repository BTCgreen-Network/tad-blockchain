...
from tad.plot_sync.receiver import Receiver
...
from tad.rpc.farmer_rpc_api import (
    FilterItem,
...
from tad.simulator.block_tools import get_plot_dir
from tad.simulator.time_out_assert import time_out_assert, time_out_assert_custom_interval
...
from tad.util.config import load_config, lock_and_load_config, save_config
...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "tad"), encode_puzzle_hash(new_ph_2, "tad"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "tad"), encode_puzzle_hash(observer_pool, "tad")
    )
    ...
    assert config["farmer"]["tad_target_address"] == encode_puzzle_hash(observer_farmer, "tad")
    assert config["pool"]["tad_target_address"] == encode_puzzle_hash(observer_pool, "tad")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "tad")
