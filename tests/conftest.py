...
# NOTE:
#       Instantiating the bt fixture results in an attempt to create the tad root directory
...
@pytest_asyncio.fixture(scope="function")
async def two_wallet_nodes_custom_spam_filtering(spam_filter_after_n_txs, tad_spam_amount):
    async for _ in setup_simulators_and_wallets(1, 2, {}, spam_filter_after_n_txs, tad_spam_amount):
        yield _
...
@pytest.fixture(scope="function")
def tmp_tad_root(tmp_path):
    """
    Create a temp directory and populate it with an empty tad_root directory.
    """
    path: Path = tmp_path / "tad_root"
    path.mkdir(parents=True, exist_ok=True)
    return path


@pytest.fixture(scope="function")
def root_path_populated_with_config(tmp_tad_root) -> Path:
    """
    Create a temp tad_root directory and populate it with a default config.yaml.
    Returns the tad_root path.
    """
    root_path: Path = tmp_tad_root
