...
from tad.wallet.cat_wallet.cat_utils import (
...
from tad.wallet.trading.offer import Offer, NotarizedPayment
...
# This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/TAD coins
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            tad_coins: List[Coin] = all_coins[None]
            ...
            # Create an TAD Offer for RED
            tad_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            tad_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                tad_requested_payments, tad_coins
            )
            tad_announcements: List[Announcement] = Offer.calculate_announcements(tad_requested_payments, driver_dict)
            tad_secured_bundle: SpendBundle = generate_secure_bundle(tad_coins, tad_announcements, 1000)
            tad_offer = Offer(tad_requested_payments, tad_secured_bundle, driver_dict)
            assert not tad_offer.is_valid()

            # Create a RED Offer for TAD
            ...
            new_offer = Offer.aggregate([tad_offer, red_offer, red_offer_2])
            ...
            # Create yet another offer of BLUE for TAD and RED
            ...
            assert new_offer.summary() == (
                ...
                {"tad": 900, str_to_tail_hash("red").hex(): 350},
