...
class TestCATTrades:
    ...
    async def test_cat_trades(self, wallets_prefarm):
        ...
        MAKER_TAD_BALANCE = initial_maker_balance - 100
        TAKER_TAD_BALANCE = initial_taker_balance - 100
        await time_out_assert(25, wallet_maker.get_confirmed_balance, MAKER_TAD_BALANCE)
        await time_out_assert(25, wallet_taker.get_unconfirmed_balance, TAKER_TAD_BALANCE)
        ...
        # tad_for_cat
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(tad_for_cat, fee=uint64(1))
        await asyncio.sleep(1)
        ...
        MAKER_TAD_BALANCE -= 2  # -1 and -1 for fee
        MAKER_NEW_CAT_BALANCE += 2
        TAKER_TAD_BALANCE += 0  # +1 and -1 for fee
        TAKER_NEW_CAT_BALANCE -= 2

        await time_out_assert(15, wallet_taker.get_unconfirmed_balance, TAKER_TAD_BALANCE)
        ...
        await time_out_assert(15, wallet_maker.get_confirmed_balance, MAKER_TAD_BALANCE)
        await time_out_assert(15, wallet_maker.get_unconfirmed_balance, MAKER_TAD_BALANCE)
        ...
        await time_out_assert(15, wallet_taker.get_confirmed_balance, TAKER_TAD_BALANCE)
        await time_out_assert(15, wallet_taker.get_unconfirmed_balance, TAKER_TAD_BALANCE)
        ...
        # cat_for_tad
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cat_for_tad)
        assert error is None
        assert success is True
        assert trade_make is not None
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
            tad_for_multiple_cat, driver_dict=driver_dict
        )
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        ...
        # multiple_cat_for_tad
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(multiple_cat_for_tad)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
        ...
        # tad_and_cat_for_cat
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(tad_and_cat_for_cat)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
    ...
    async def test_trade_cancellation(self, wallets_prefarm):
        ...
        tad_to_cat_amount = uint64(100)

        async with wallet_node_maker.wallet_state_manager.lock:
            cat_wallet_maker: CATWallet = await CATWallet.create_new_cat_wallet(
                wallet_node_maker.wallet_state_manager, wallet_maker, {"identifier": "genesis_by_id"}, tad_to_cat_amount
            )
        ...
        await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, tad_to_cat_amount)
        await time_out_assert(15, cat_wallet_maker.get_unconfirmed_balance, tad_to_cat_amount)
        maker_funds -= tad_to_cat_amount
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(tad_for_cat)
        assert error is None
        assert success is True
        assert trade_make is not None
