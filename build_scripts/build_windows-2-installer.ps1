...
Set-Location -Path "tad-blockchain-gui\packages\gui" -PassThru
...
electron-builder build --win --x64 --config.productName="Tad"
...
If ($env:HAS_SECRET) {
   ...
   Write-Output "   ---"
   signtool.exe verify /v /pa .\dist\TadSetup-$packageVersion.exe
   }   Else    {
...
Write-Output "   ---"
Copy-Item ".\dist\win-unpacked" -Destination "$env:GITHUB_WORKSPACE\tad-blockchain-gui\Tad-win32-x64" -Recurse
mkdir "$env:GITHUB_WORKSPACE\tad-blockchain-gui\release-builds\windows-installer" -ea 0
Copy-Item ".\dist\TadSetup-$packageVersion.exe" -Destination "$env:GITHUB_WORKSPACE\tad-blockchain-gui\release-builds\windows-installer"
