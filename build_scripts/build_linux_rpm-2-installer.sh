...
# Builds CLI only rpm
CLI_RPM_BASE="tad-blockchain-cli-$TAD_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/tad"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/tad/"
ln -s ../../opt/tad/tad "dist/$CLI_RPM_BASE/usr/bin/tad"
...
fpm -s dir -t rpm \
  ...
  --name tad-blockchain-cli \
  --license Apache-2.0 \
  --version "$TAD_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "Tad is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
...
cd ../tad-blockchain-gui/packages/gui || exit 1
...
echo electron-builder build --linux rpm "${OPT_ARCH}" \
  --config.productName="${PRODUCT_NAME}" --config.linux.desktop.Name="Tad Blockchain" \
  --config.rpm.packageName="tad-blockchain"
...
GUI_RPM_NAME="tad-blockchain-${TAD_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${TAD_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
