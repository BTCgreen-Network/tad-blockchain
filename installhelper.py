...
# Gets the version from the result of "tad version"
...
def get_tad_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["tad", "version"], capture_output=True)
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/tad-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/tad-blockchain-gui/packages/gui/package.json")
