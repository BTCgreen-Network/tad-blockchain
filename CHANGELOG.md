...
## 1.4.0 Tad blockchain 2022-6-29
...
- New coin selection algorithm based on bitcoin knapsack. Previously tad selected the largest coin
...
- Updated warning about `TAD_ROOT` being set when running init
...
## 1.3.5 Tad blockchain 2022-5-11
...
## 1.3.4 Tad blockchain 2022-4-19
...
- Added `-n`/`--new-address` option to `tad wallet get_address`
...
- Fixed typo in command line argument parsing for `tad db validate`
- Improved backwards compatibility for node RPC calls `get_blockchain_state` and `get_additions_and_removals`
- Fixed issue where `--root_path` option was not honored by `tad configure` CLI command
- Fixed cases where node DB was not created initially using v2 format
- Improved error messages from `tad db upgrade`
- Capitalized display of `Rpc` -> `RPC` in `tad show -s` by (Thanks @hugepants)
...
- Minor output formatting/enhancements for `tad wallet show`
- Fixed typo and index issues in wallet database
- Used the rust clvm version instead of python in more places
- Fixed trailing bytes shown in CAT asset ID row when using `tad wallet show`
...
- You cannot install and run tad blockchain using the macOS packaged DMG on macOS Mojave (10.14).
...
## 1.3.3 Tad blockchain 2022-4-02
...
- Fixed exception when `tad keys migrate` is run without needing migration.
...
- Added checks to ensure wallet address prefixes are either `tad` or `ttad`.
...
## 1.3.0 Tad blockchain 2022-3-07
...
- Added new RPC, get_version, to the daemon to return the version of Tad (Thanks @dkackman).
- Added new config.yaml setting, reserved_cores, to specify how many cores Tad will not use when launching process pools. Using 0 will allow Tad to use all cores for process pools. Set the default to 0 to allow Tad to use all cores. This can result in faster syncing and better performance overall especially on lower-end CPUs like the Raspberry Pi4.
- Added new RPC, get_logged_in_fingerprint, to the wallet to return the currently logged in fingerprint.
- Added new CLI option, tad keys derive, to allow deriving any number of keys in various ways. This is particularly useful to do an exhaustive search for a given address using tad keys derive search.
...
- CLI option tad show will display the currently selected network (mainnet or testnet).
- CLI option tad plots check will display the Pool Contract Address for Portable (PlotNFT) plots.
...
- Fixed issues with displaying IPv6 address in CLI with tad show -c.
- Thanks to @chuwt for fix to looping logic during node synching.
- Fixed the tad-blockchain RPM to set the permission of chrome-sandbox properly.
...
- Fixed display info in CLI tad show -c when No Info should be displayed.
...
- When you close the Tad app, regardless if you are in farmer mode or wallet, the content on the exit dialog isn’t correct.
- If you start with wallet mode and then switch to farmer mode and back to wallet mode, the full node will continue to sync in the background. To get the full node to stop syncing after switching to wallet mode, you will need to close the Tad and relaunch the Tad app.
...
## 1.2.11 Tad blockchain 2021-11-4
...
- Integrated the Bladebit plotter to CLI and GUI. Thanks @harold-b for all your hard work on this, and welcome again to the Tad Network team!
- Added the Madmax plotter to CLI and GUI. Thanks @madMAx43v3r for your support!
- Added option to configure your node to testnet using to `tad init --testnet`.
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other tad services.
...
- Updated the old comment on tad/util/streamable.py with newer developer documentation.
...
- Thanks @jcteng for fixing a bug on the Tad DID wallet that showed 'mojo:'' instead of 'mojo'.

## 1.2.9 Tad blockchain 2021-10-01

### Changed

- Changed "About" section in client to indicate correct release version.

## 1.2.8 Tad blockchain 2021-09-30
...
- Added support for DID, our decentralized identity solution, as a building block toward Tad's broader set of DID capabilities.
...
- Several clvm_rs updates to support our upcoming Tad Asset Token (CAT) standard.
...
- Added a py.typed file that allows other projects that pip install tad-blockchain to type check using our functions with mypy.
...
## 1.2.3 Tad blockchain 2021-07-26
...
## 1.2.2 Tad blockchain 2021-07-13
...
## 1.2.0 Tad blockchain 2021-07-07
...
- All Tad repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `tad plotnft` to manage pools.
...
- Correctly display private key in `tad keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `tad keys show`.
...
- Thank you to @TadMineJP for various improvements.
...
- `tad farm summary` better handles wallet errors.
...
### 1.1.7 Tad Blockchain 2021-06-05
...
## 1.1.6 Tad Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /tad/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `tad configure -t true|false`).
...
## 1.1.5 Tad Blockchain 2021-05-09
...
## 1.1.4 Tad Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `tad keys show`. Use `tad keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Tad Blockchain 2021-05-01
...
## 1.1.2 Tad Blockchain 2021-04-24
...
## 1.1.1 Tad Blockchain 2021-04-21
...
## 1.1.0 Tad Blockchain 2021-04-21
...
## 1.0.5 Tad Blockchain 2021-04-14
...
## 1.0.4 Tad Blockchain 2021-04-12
...
- The `src` directory in tad-blockchain has been changed to `tad` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `tad/`
...
## 1.0.3 Tad Blockchain 2021-03-30
...
- An incorrect merge brought in unreleased features and broke `tad keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of chiapos 1.0.0 as well.

## 1.0.2 Tad Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [chiapos](https://github.com/Tad-Network/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- @jespino changed `tad keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `tad keys show`.
...
## 1.0.1 Tad Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `tad keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/tad-blockchain/).
...
- Users can now pip install e.g. tad-blockchain==1.0.1 on most platforms.
- Sometimes the GUI had an error regarding MainWindow.

## 1.0.0 First Release of Tad Blockchain 2021-03-17

### Added

- This is the first production release of the Tad Blockchain. This can be installed and will wait for the green flag that will be dropped at approximately 7AM PDST (14:00 UTC) on Friday March 19, 2021. All farming rewards from that point forward will be considered valid and valuable TAD. There is a six week lock on all transactions. During those six weeks farmers will be earning their farming rewards but those rewards can not be spent.
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/tad-blockchain).
...
- Migration with TAD_ROOT set does not crash tad init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TCAC Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.tadcoins.com/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `tad farm summary` will now display TCAC or CAC as appropriate.
...
- `tad farm summary` displayed some incorrect amounts.
...
- Remove `tad plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `tad keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
...
- PRs and commits to the tad-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TCAC farmed.
- `tad farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `tad wallet` had various bugs.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of tad-blockchain, etc., instead of in blspy.
...
- Various typos and style clean ups were made to the Click CLI implementation. `tad -upnp f` was added to disable uPnP.
- `tad plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major tadlisp changes. TCAC from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- `tad configure -log-level` was broken.
...
- 'tad wallet send' now takes in TCAC or CAC as units instead of mojos.
...
- You can now send and receive transactions with the command line. Try `tad wallet -h` to learn more. Also, `tad wallet` now requires a third argument of `show`, therefor you will use `tad wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [tad blockchain gui](https://crowdin.com/project/tad-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `tad init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `tad init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- The cli wallet, `tad wallet`, was incorrectly displaying TCAC balances as if they were Coloured Coins.
...
- `tad plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- `tad plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `tad show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `tad show` handles sub block 0 better.
...
- `tad plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `tad plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `tad show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `tad configure --enable-upnp`.
...
- `tad configure` now lets you enable or disable uPnP.
...
- You can now specify which private key to use for `tad plots create`. After obtaining the fingerprint from `tad keys show`, try `tad plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the Tad application set to launch on start.
...
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.tad/VERSION/config/`
...
- `tad show -s` now is less brain dead when a node is initially starting to sync.
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/Tad-Network/chiapos/releases/tag/0.12.32).
...
- The rpc interfaces of all tad services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test tad since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve tad-blocktadn's Pypi entry. Thanks to @altendky for this pull request.
...
- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test tad are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.tad` directory.
...
- `tad init` now automatically discovers previous releases in each new release.

### Fixed

- `tad show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `tad show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now Tad Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The Tad UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (tad run farmer-only, wallet-only, timelord-only)
...
- `tad show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `tad show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the Tad simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the Tad implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test tad between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test tad from the earlier two releases however.
...
- `tad show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Tad)
...
- See wallet balances in command line: `tad show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `tad keys sign` and `tad keys verify` to allow farmers to certify their ownership of keys.
...
- `tad start wallet-server` changed to `tad start wallet`, for consistency.
...
- Removed legacy scripts such as tad-stop-server, tad-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/Tad-Network/bls-signatures) to use the
...
- The tad executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./tad -h` from
`~\AppData\Local\Tad-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/Tad.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.
...
"tad plots create" and "tad plots check".
- Add plot directories to config.yaml from the cli with "tad plots add".
...
- The Windows installer is now signed by a Tad Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.tad/test-plots/`
- We have completely refactored all networking code towards making each Tad service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make tad start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Tad Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting tad now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'tad keys' command line.
...
- We added a `tad keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the tad-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `tad-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `tad-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `tad-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `tad-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `tad init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `tad start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `tad-start-harvester`.
...
- The user interface now refers to tad coins with their correct currency code of CAC.
...
- There was a regression in Tad Proof of Space ([chiapos](https://github.com/Tad-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `tad init` then sync on first start will happen automatically. "\$TAD_ROOT" users will need to delete `$TAD_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- The script `tad start wallet-gui` has been chaned to `tad start wallet` which launches but the GUI and server on MacOS and Linux. `tad start wallet-server` remains for WSL 2 and Windows native.
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `tad stop all` command sometimes fails, use `tad-stop-all` instead. In windows, use the task manager to stop the servers.
...
- We added full block lookup to `tad show`.

### Changed

- `tad-restart-harvester` has been renamed from `tad-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.tad/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the Tad blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/Tad-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- The libraries tad-pos, tad-fast-vdf, and tad-bip-158 have been moved to their own repositories: [chiapos](https://github.com/Tad-Network/chiapos), [chiavdf](https://github.com/Tad-Network/chiavdf), and [chaibip158](https://github.com/Tad-Network/chiabip158). They are brought in by tad-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Tad-Network/bls-signatures).
...
- New AJAX based full node UI. To access go to [http://127.0.0.1:4555/index.html](http://127.0.0.1:4555/index.html) with any modern web browser on the same machine as the full node.
...
- Tad fullnode, farmer and harvester now default to logging to tad.log in the tad-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to tad_full node, tad_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new tad services were started.
