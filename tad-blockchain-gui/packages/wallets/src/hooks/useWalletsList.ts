...
import { useGetWalletsQuery, useGetStrayCatsQuery, useGetCatListQuery, useAddCATTokenMutation } from '@tad/api-react';
import { useShowError } from '@tad/core';
  ...
  const list = useMemo(() => {
    ...
    let tokens = [
      ...baseWallets.map((wallet: Wallet) => ({
        ...
        name: wallet.type === WalletType.STANDARD_WALLET ? 'Tad' : wallet.meta?.name ?? wallet.name,
      })),
