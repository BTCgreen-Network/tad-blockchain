import { AlertDialog, Card, Flex, tadToMojo, mojoToTadLocaleString } from '@tad/core';
...
function IncompleteCard(props) {
  ...
  function submit() {
    ...
    const hetadeck = /[\da-f]+$/gi;

    if (!hetadeck.test(ip_val) || ip_val.value === '') {
    ...
    const tadper_input = ip_parsed.limit;
    ...
    const tadper_value = Number.parseInt(Number(tadper_input));
    const origin_parsed = JSON.parse(origin_input);
    dispatch(rl_set_user_info_action(id, interval_value, tadper_value, origin_parsed, admin_pubkey_input));
  }
      ...
      <Card title={<Trans>Rate Limited Info</Trans>}>
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>Spending Limit (tad per interval): {mojoToTadLocaleString(limit)}</Trans>
              </Typography>
            </Box>
          </Box>
        </Grid>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToTadLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function SendCard(props) {
  ...
  function send() {
    ...
    const amount = tadToMojo(amount_input.value);
    const fee = tadToMojo(fee_input.value);
