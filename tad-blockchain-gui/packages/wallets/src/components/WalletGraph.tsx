import { TransactionType, WalletType } from '@tad/api';
import type { Transaction } from '@tad/api';
import { useGetWalletBalanceQuery } from '@tad/api-react';
import { mojoToTad, mojoToCAT, blockHeightToTimestamp } from '@tad/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToTad(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: (walletType === WalletType.CAT ? mojoToCAT(balance) : mojoToTad(balance)).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToTad(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: walletType === WalletType.CAT ? mojoToCAT(start) : mojoToTad(start).toString(), // bignumber is not supported by react
    });
  });
