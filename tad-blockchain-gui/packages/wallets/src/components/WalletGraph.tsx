...
import { TransactionType } from '@tad/api';
import type { Transaction } from '@tad/api';
import { mojoToTad, blockHeightToTimestamp } from '@tad/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, mojoToTad(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToTad(balance).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, mojoToTad(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToTad(start).toString, // bignumber is not supported by react
    });
  });
