import { WalletType, TransactionType, toBech32m } from '@tad/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
    ...
    field: (row: Row, metadata) => {
      ...
      return (
        <>
          ...
          &nbsp;
          <strong>
            <FormatLargeNumber value={type === WalletType.CAT ? mojoToCAT(row.amount) : mojoToTad(row.amount)} />
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToTad(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
