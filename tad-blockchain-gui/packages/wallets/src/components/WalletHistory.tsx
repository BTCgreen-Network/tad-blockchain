...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToTadLocaleString, mojoToCATLocaleString } from '@tad/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@tad/api-react';
import styled from 'styled-components';
import type { Row } from '@tad/core';
import { WalletType, TransactionType } from '@tad/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToTadLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToTadLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
