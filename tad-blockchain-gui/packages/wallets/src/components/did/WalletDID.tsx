import { AlertDialog, Card, Flex, Loading, Dropzone, mojoToTadLocaleString } from '@tad/core';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">{mojoToTadLocaleString(props.balance)} TTAD</Typography>
        </Box>
      </Box>
    </Grid>
  );
...
function CreateAttest(props) {
  ...
  function createAttestPacket() {
    ...
    if (address.substring(0, 12) === 'tad_addr://') {
...
function CashoutCard(props) {
  ...
  function cashout() {
    ...
    if (puzzlehash.slice(0, 12) === 'tad_addr://') {
