import { AlertDialog, tadToMojo } from '@tad/core';
...
export function CreateRLAdminWallet() {
  ...
  function create() {
    ...
    if (
      tadper_input.value === '' ||
      Number(tadper_input.value) === 0 ||
      !Number(tadper_input.value) ||
      isNaN(Number(tadper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const tadper = tadToMojo(tadper_input.value);
    const tadper_value = Number.parseInt(Number(tadper));
    ...
    // var fee = tadToMojo(fee_input.value);
    // TODO(lipa): send fee to server
    // const fee_value = parseInt(Number(fee));
    dispatch(create_rl_admin_action(interval_value, tadper_value, userpubkey, amount_value));
  }
