...
import { type OfferSummaryRecord  } from '@tad/api';
import {
  Flex,
  StateColor,
  mojoToTad,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries[0][0].toLowerCase() === 'tad' ? Number(mojoToTad(makerEntries[0][1])) : Number(mojoToCAT(makerEntries[0][1]));
  const takerAmount = takerEntries[0][0].toLowerCase() === 'tad' ? Number(mojoToTad(takerEntries[0][1])) : Number(mojoToCAT(takerEntries[0][1]));
