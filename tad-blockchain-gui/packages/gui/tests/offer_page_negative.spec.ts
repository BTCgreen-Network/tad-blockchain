...
test('Confirm Error message when fields are not completed on Offer Page', async () => {
  ...
  await page.locator('text=Amount *TTAD >> input[type="text"]').fill('0.05');
  ...
  await page.locator('text=Create an NFT OfferBuy an NFTSell an NFTYou will offerAmount *TTAD50,000,000,000 >> button').first().click();
  ...
  await page.locator('text=You will offerAsset Type *​Amount *TTAD >> input[type="text"]').fill('0.00000000005');


  // And I complete Amount under heading In exchange for
  await page.locator('text=In exchange forAsset Type *​Amount *TTAD >> input[type="text"]').fill('0.00000000005');
