...
const log = debug('tad-gui:offers');
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50029, // Pairing doesn't exist e.g. TAD/RandoCoin
...
async function postToSpacescan(offerData: string, testnet: boolean): Promise<string> {
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'ttad' : 'tad'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
  ...
  return `https://www.spacescan.io/${testnet ? 'ttad' : 'tad'}/offer/${id}`;
}
...
const KeybaseTeamName = 'tad_offers';
...
function OfferShareKeybaseDialog(props: OfferShareServiceDialogProps) {
  ...
  const teamName = testnet ? 'testtadoffersdev' : KeybaseTeamName;
