...
import type { OfferSummaryAssetInfo, OfferSummaryInfos, OfferSummaryRecord } from '@tad/api';
import { mojoToCAT, mojoToTad, mojoToCATLocaleString, mojoToTadLocaleString } from '@tad/core';
...
export function summaryStringsForNFTOffer(
  ...
  // TODO: Remove 1:1 NFT <--> TAD assumption
...
export function formatAmountForWalletType(amount: string | number, walletType: WalletType, locale?: string): string {
  if (walletType === WalletType.STANDARD_WALLET) {
    return mojoToTadLocaleString(amount, locale);
  }
...
export function getNFTPriceWithoutRoyalties(
  ...
  for (const assetType of [OfferAsset.TOKEN, OfferAsset.TAD]) {
    ...
    if (assetId) {
      ...
      if (amountInMojos) {
        const amountInTokens = assetType === OfferAsset.TAD ? mojoToTad(amountInMojos) : mojoToCAT(amountInMojos);
