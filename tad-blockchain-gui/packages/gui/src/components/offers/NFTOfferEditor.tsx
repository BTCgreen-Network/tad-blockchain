...
import type { NFTInfo, Wallet } from '@tad/api';
import {
...
/*        An NFT offer consists of a single NFT being offered for TAD         */
/* ========================================================================== */
...
function NFTOfferConditionalsPanel(props: NFTOfferConditionalsPanelProps) {
  ...
  const amount = methods.watch('tadAmount');
  ...
  const offerElem =
    tab === NFTOfferExchangeType.NFTForTAD ? nftElem : amountElem;
  const takerElem =
    tab === NFTOfferExchangeType.NFTForTAD ? amountElem : nftElem;
  ...
  function handleAmountChange(amount: string) {
    methods.setValue('tadAmount', amount);
  }
  ...
  return (
    ...
    >
      ...
      >
        <Tab
          value={NFTOfferExchangeType.NFTForTAD}
          label={<Trans>NFT for TAD</Trans>}
          disabled={disabled}
        />
        <Tab
          value={NFTOfferExchangeType.TADForNFT}
          label={<Trans>TAD for NFT</Trans>}
          disabled={disabled}
        />
      </Tabs>
      <Grid container>
        <Flex flexDirection="column" flexGrow={1} gap={3}>
          ...
          {tab === NFTOfferExchangeType.TADForNFT && (
            <Flex flexDirection="column" gap={2}>
              ...
              <>
                <Flex flexDirection="column" gap={0.5}>
                  <Typography variant="body1" color="textSecondary">
                    {tab === NFTOfferExchangeType.NFTForTAD ? (
...
/* ========================================================================== */
/*                              NFT Offer Editor                              */
/*             Currently only supports a single NFT <--> TAD offer            */
/* ========================================================================== */
...
function buildOfferRequest(
  ...
  const baseMojoAmount: BigNumber = tadToMojo(tadAmount);
  const mojoAmount =
    exchangeType === NFTOfferExchangeType.NFTForTAD
      ? baseMojoAmount
      : baseMojoAmount.negated();
  const feeMojoAmount = tadToMojo(fee);
  const nftAmount = exchangeType === NFTOfferExchangeType.NFTForTAD ? -1 : 1;
  const tadWalletId = 1;
  ...
  return [
    {
      ...
      [tadWalletId]: mojoAmount,
    },
    exchangeType === NFTOfferExchangeType.TADForNFT ? driverDict : undefined,
    // driverDict,
    feeMojoAmount,
  ];
}
...
export default function NFTOfferEditor(props: NFTOfferEditorProps) {
  ...
  function validateFormData(
    ...
    const { exchangeType, nftId, tadAmount, fee } = unvalidatedFormData;
  ...
  async function handleSubmit(unvalidatedFormData: NFTOfferEditorFormData) {
    ...
    const { exchangeType, launcherId, tadAmount, fee } = formData;

    if (exchangeType === NFTOfferExchangeType.NFTForTAD) {
      const haveNFT =
