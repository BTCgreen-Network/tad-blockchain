...
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToTad, mojoToCAT } from '@tad/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['tad', 'ttad'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToTad(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['tad', 'ttad'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToTad(takerEntries[0][1])
