import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@tad/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@tad/api';
import { useCalculateRoyaltiesForNFTsQuery, useGetNFTsByNFTIDsQuery, useGetWalletsQuery } from '@tad/api-react';
import { catToMojo, tadToMojo } from '@tad/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedTAD = useWatch({
    name: 'requested.tad',
  });

  const offeredTAD = useWatch({
    name: 'offered.tad',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedTAD ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('tad', tadToMojo(amount))),
    ...(requestedTokens ?? [])
