import { Flex } from '@tad/core';
import { Offering, Requesting } from '@tad/icons';
...
import OfferBuilderTADSection from './OfferBuilderTADSection';
...
export default function OfferBuilderTradeColumn(props: OfferBuilderTradeColumnProps) {
  ...
  const showTAD = !readOnly || !!tad.length;
  ...
  const mutedTAD = nfts.length || tokens.length;
  const mutedTokens = tad.length || nfts.length;
  const mutedNFTs = tad.length || tokens.length;
  ...
  return (
    <Flex flexDirection="column" gap={3}>
      ...
      >
        {showTAD && <OfferBuilderTADSection name={`${name}.tad`} offering={offering} muted={mutedTAD} />}
