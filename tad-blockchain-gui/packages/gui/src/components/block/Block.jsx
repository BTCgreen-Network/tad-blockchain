...
import { toBech32m } from '@tad/api';
import { useGetBlockQuery, useGetBlockRecordQuery  } from '@tad/api-react'
...
export default function Block() {
  ...
  const poolReward = mojoToTad(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToTad(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const tadFees = blockRecord.fees !== undefined
    ? mojoToTad(blockRecord.fees)
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.tadexplorer.com/blockchain/puzzlehash/${blockRecord.farmerPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.tadexplorer.com/blockchain/puzzlehash/${blockRecord.poolPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: tadFees ? `${tadFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Tad blockchain
            </Trans>
          </Back>
        }
        action={
