...
export function getUserDataDir(): string {
  const tadRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(tadRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const tadRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${tadRootUserDataPath}`);
  app.setPath('userData', tadRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(tadRootUserDataPath, 'Local Storage', 'leveldb');
