...
import { tadToMojo, catToMojo } from '@tad/core';
...
export default async function offerBuilderDataToOffer(
  ...
  const {
    offered: { tad: offeredXch = [], tokens: offeredTokens = [], nfts: offeredNfts = [], fee: [firstFee] = [] },
    requested: { tad: requestedXch = [], tokens: requestedTokens = [], nfts: requestedNfts = [] },
  } = data;

  const usedNFTs: string[] = [];

  const feeInMojos = firstFee ? tadToMojo(firstFee.amount) : new BigNumber(0);
  ...
  await Promise.all(
    offeredXch.map(async (tad) => {
      const { amount } = tad;
      if (!amount || amount === '0') {
        throw new Error(t`Please enter an TAD amount`);
      }
  ...
  requestedXch.forEach((tad) => {
    ...
    walletIdsAndAmounts[wallet.id] = tadToMojo(amount);
  });
