...
const getTadVersion = () => {
  let version = null;
  const exePath = getExecutablePath('tad');
  // first see if we can get a tad exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or tad is on the path
    try {
      version = child_process
