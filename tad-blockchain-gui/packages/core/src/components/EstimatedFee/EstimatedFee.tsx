...
import { useGetFeeEstimateQuery } from '@tad/api-react';
import {
  FormControl,
  IconButton,
  InputLabel,
...
import mojoToTadLocaleString from '../../utils/mojoToTadLocaleString';
import Fee from '../Fee';
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const txCostEstimates = {
    walletSendTAD: Math.floor(maxBlockCostCLVM / 1170),
  ...
  function formatEst(number, multiplier, locale) {
    ...
    let formatNum = mojoToTadLocaleString(num, locale);
    return formatNum;
  }
        ...
        >
          {estList.map((option) => (
            <MenuItem value={String(option.estimate)} key={option.time}>
              ...
              >
                <Flex>
                  <Trans>{option.estimate} TTAD</Trans>
                </Flex>
