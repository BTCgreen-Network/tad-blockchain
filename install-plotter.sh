...
get_madmax_filename() {
  ...
  echo "${TAD_PLOT}-${MADMAX_VER}-${SUFFIX}"
}
...
if [ "$PLOTTER" = "bladebit" ]; then
  ...
  mv -f "${madmax_filename}" tad_plot
  chmod 755 tad_plot
  echo "Successfully installed madmax to $(pwd)/tad_plot"
  ...
  mv -f "${madmax_filename}" tad_plot_k34
  chmod 755 tad_plot_k34
  echo "Successfully installed madmax for k34 to $(pwd)/tad_plot_k34"
else
  usage
fi
