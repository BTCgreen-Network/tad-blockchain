...
from tad.types.weight_proof import SubEpochData, WeightProof
from tad.util.byte_types import hexstr_to_bytes
from tad.util.chunks import chunks
from tad.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from tad.util.default_root import STANDALONE_ROOT_PATH
from tad.util.ints import uint32, uint64
from tad.util.keychain import Keychain, KeyringIsLocked
...
class WalletNode:
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSTadConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def fetch_puzzle_solution(self, peer: WSTadConnection, height: uint32, coin: Coin) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSTadConnection] = None
    ) -> List[CoinState]:
        all_nodes = self.server.connection_by_type[NodeType.FULL_NODE]
        if len(all_nodes.keys()) == 0:
            raise ValueError("Not connected to the full node")
    ...
    async def fetch_children(
        self, peer: WSTadConnection, coin_name: bytes32, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
