...
from tad.consensus.coinbase import farmer_parent_id, pool_parent_id
from tad.consensus.constants import ConsensusConstants
from tad.data_layer.data_layer_wallet import DataLayerWallet
from tad.data_layer.dl_wallet_store import DataLayerStore
...
from tad.wallet.cat_wallet.cat_utils import construct_cat_puzzle, match_cat_puzzle
from tad.wallet.cat_wallet.cat_wallet import CATWallet
from tad.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
...
from tad.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, create_fullpuz, match_did_puzzle
from tad.wallet.key_val_store import KeyValStore
from tad.wallet.nft_wallet.nft_info import NFTWalletInfo
from tad.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
from tad.wallet.nft_wallet.nft_wallet import NFTWallet
from tad.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from tad.wallet.notification_manager import NotificationManager
from tad.wallet.outer_puzzles import AssetType
from tad.wallet.puzzle_drivers import PuzzleInfo
from tad.wallet.puzzles.cat_loader import CAT_MOD, CAT_MOD_HASH
...
from tad.wallet.util.address_type import AddressType
from tad.wallet.util.compute_hints import compute_coin_hints
from tad.wallet.util.transaction_type import TransactionType
from tad.wallet.util.wallet_sync_utils import last_change_height_cs, PeerRequestException
...
from tad.wallet.wallet_retry_store import WalletRetryStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSTadConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        tad_spam_amount = self.config.get("tad_spam_amount", 1000000)
        ...
        small_unspent_count = await self.coin_store.count_small_unspent(tad_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < tad_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSTadConnection) -> Optional[bytes32]:
        # Get minter DID
