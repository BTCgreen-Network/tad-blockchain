...
class Wallet:
    ...
    async def create_spend_bundle_relative_tad(self, tad_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        ...
        if tad_amount < 0:
            utxos = await self.select_coins(uint64(abs(tad_amount)), exclude)
        else:
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(tad_amount), "memos": []}
                ]
    ...
    # WSTadConnection is only imported for type checking
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSTadConnection
