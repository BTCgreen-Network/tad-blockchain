...
from tad.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from tad.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            tad_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        tad_tx = dataclasses.replace(tad_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(tad_tx)
    ...
    async def create_tandem_tad_tx(
        ...
        if fee > amount_to_claim:
            tad_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            origin_id = list(tad_coins)[0].name()
            ...
            message = None
            for spend in tad_tx.spend_bundle.coin_spends:
            ...
            tad_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            selected_amount = sum([c.amount for c in tad_coins])
            tad_tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some TAD as part of this
        regular_tad_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_tad_to_claim = payment_amount

        need_tad_transaction = (fee > 0 or regular_tad_to_claim > 0) and (fee - regular_tad_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_tad_transaction:
                    if fee > regular_tad_to_claim:
                        tad_tx, _ = await self.create_tandem_tad_tx(
                            fee,
                            uint64(regular_tad_to_claim),
                    ...
                    elif regular_tad_to_claim > fee:
                        tad_tx, _ = await self.create_tandem_tad_tx(
                            fee, uint64(regular_tad_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        tad_spend_bundle = SpendBundle([], G2Element())
        if tad_tx is not None and tad_tx.spend_bundle is not None:
            tad_spend_bundle = tad_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        if tad_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=tad_tx.confirmed_at_height,
                    created_at_time=tad_tx.created_at_time,
                    to_puzzle_hash=tad_tx.to_puzzle_hash,
                    amount=tad_tx.amount,
                    fee_amount=tad_tx.fee_amount,
                    confirmed=tad_tx.confirmed,
                    sent=tad_tx.sent,
                    spend_bundle=None,
                    additions=tad_tx.additions,
                    removals=tad_tx.removals,
                    wallet_id=tad_tx.wallet_id,
                    sent_to=tad_tx.sent_to,
                    trade_id=tad_tx.trade_id,
                    type=tad_tx.type,
                    name=tad_tx.name,
                    memos=[],
                )
