...
from tad.wallet.derive_chives_keys import chives_master_sk_to_pool_sk
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("tad_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("tad_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("tad_target_address") or all_targets[-2] == config["pool"].get(
                    "tad_target_address"
                ):
        ...
        if "tad_target_address" not in config["farmer"]:
            print(
                f"Setting the tad destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
        ...
        if updated_target:
            print(
                f"To change the TAD destination addresses, edit the `tad_target_address` entries in"
...
def init(
    ...
    if create_certs is not None:
        ...
        return tad_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
...
def tad_init(
    ...
    *,
    ...
    """
    tad_root = os.environ.get("TAD_ROOT", None)
    if tad_root is not None:
        print(f"TAD_ROOT is set to {tad_root}")
