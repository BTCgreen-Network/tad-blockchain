...
from tad.cmds.passphrase_funcs import obtain_current_passphrase
...
from tad.util.errors import KeychainNotSet
from tad.util.config import load_config
from tad.util.default_root import DEFAULT_ROOT_PATH
from tad.util.errors import KeychainException
from tad.util.file_keyring import MAX_LABEL_LENGTH
from tad.util.ints import uint32
from tad.util.keychain import Keychain, bytes_to_mnemonic, generate_mnemonic, mnemonic_to_seed
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'tad keys generate'")
...
def derive_sk_from_hd_path(master_sk: PrivateKey, hd_path_root: str) -> Tuple[PrivateKey, str]:
    ...
    from tad.wallet.derive_keys import _derive_path, _derive_path_unhardened
...
async def migrate_keys(root_path: Path, forced: bool = False) -> bool:
    ...
    deprecation_message = (
        ...
        "You need to migrate your keyring to continue using Tad.\n"
    )
    ...
    if Keychain.needs_migration():
        ...
        if len(keys_to_migrate) > 0:
            ...
            if not prompt_yes_no("Migrate these keys?"):
                ...
                print("Migration aborted, can't run any tad commands.")
                return False
