...
from tad.cmds.cmds_util import execute_with_wallet
from tad.cmds.coins import coins_cmd
from tad.cmds.plotnft import validate_fee
...
@click.option(
    "-ma",
    "--min-coin-amount",
    help="Ignore coins worth less then this much TAD or CAT units",
    type=str,
    required=False,
    default="0",
)
@click.option(
    "-l",
    "--max-coin-amount",
    help="Ignore coins worth more then this much TAD or CAT units",
    type=str,
    required=False,
    default="0",
)
...
def address_sign_message(wallet_rpc_port: Optional[int], fingerprint: int, address: str, hex_message: str) -> None:
    extra_params: Dict[str, Any] = {"address": address, "message": hex_message, "type": AddressType.TAD}
    import asyncio
...
@wallet_cmd.command("make_offer", short_help="Create an offer of TAD/CATs for TAD/CATs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in TAD.",
    type=str,
    default="0",
    show_default=True,
