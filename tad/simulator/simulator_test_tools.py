...
from tad.ssl.create_ssl import create_all_ssl
...
from tad.util.errors import KeychainFingerprintExists
...
def create_config(
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "ttad")
...
async def get_full_tad_simulator(
    ...
    """
    A tad root Path is required.
    The tad root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    with Lockfile.create(daemon_launch_lock_path(tad_root)):
        ...
        async for simulator in start_simulator(tad_root, automated_testing):
            yield simulator, tad_root, config, mnemonic, fingerprint, keychain
