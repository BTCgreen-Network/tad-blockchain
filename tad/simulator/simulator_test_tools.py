...
from tad.cmds.init_funcs import create_all_ssl
from tad.consensus.coinbase import create_puzzlehash_for_pk
from tad.daemon.server import WebSocketServer, daemon_launch_lock_path
...
from tad.simulator.ssl_certs import (
...
from tad.simulator.start_simulator import async_main as start_simulator_main
from tad.types.blockchain_format.sized_bytes import bytes32
from tad.util.bech32m import encode_puzzle_hash
from tad.util.config import create_default_tad_config, load_config, save_config
from tad.util.ints import uint32
from tad.util.keychain import Keychain
from tad.util.lock import Lockfile
...
def create_config(
    ...
    # create tad directories
    create_default_tad_config(tad_root)
    create_all_ssl(
        tad_root,
    ...
    # load config
    config = load_config(tad_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(fingerprint), "ttad")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(tad_root, "config.yaml", config)
    return config


async def start_simulator(tad_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    service = await start_simulator_main(True, automated_testing, root_path=tad_root)
    await service.start()
...
async def get_full_tad_simulator(
    automated_testing: bool = False, tad_root: Optional[Path] = None, config: Optional[Dict[str, Any]] = None
) -> AsyncGenerator[Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int], None]:
    """
    A tad root directory can be provided, otherwise a temporary one is created.
    This test can either be run in automated mode or not, which determines which mode block tools run in.
    This test is fully interdependent and can be used without the rest of the tad test suite.
    Please refer to the documentation for more information.
    """
    # Create and setup temporary tad directories.
    if tad_root is None:
        tad_root = Path(tempfile.TemporaryDirectory().name)
    ...
    crt_path = tad_root / config["daemon_ssl"]["private_crt"]
    key_path = tad_root / config["daemon_ssl"]["private_key"]
    ca_crt_path = tad_root / config["private_ssl_ca"]["crt"]
    ca_key_path = tad_root / config["private_ssl_ca"]["key"]
    with Lockfile.create(daemon_launch_lock_path(tad_root)):
        shutdown_event = asyncio.Event()
        ws_server = WebSocketServer(tad_root, ca_crt_path, ca_key_path, crt_path, key_path, shutdown_event)
        await ws_server.setup_process_global_state()
        await ws_server.start()  # type: ignore[no-untyped-call]

        async for simulator in start_simulator(tad_root, automated_testing):
            yield simulator, tad_root, config, mnemonic, fingerprint
