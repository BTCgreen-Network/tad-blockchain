...
from tad.cmds.init_funcs import check_keys, tad_init, tad_full_version_str
...
from tad.plotters.plotters import get_available_plotters
...
from tad.util.beta_metrics import BetaMetricsLogger
from tad.util.tad_logging import initialize_service_logging
from tad.util.config import load_config
from tad.util.errors import KeychainRequiresMigration, KeychainCurrentPassphraseIsInvalid
...
from tad.util.lock import Lockfile, LockfileError
from tad.util.network import WebServer
...
if getattr(sys, "frozen", False):
    name_map = {
        "tad": "tad",
        "tad_data_layer": "start_data_layer",
        "tad_data_layer_http": "start_data_layer_http",
        ...
        "tad_seeder": "start_seeder",
        "tad_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Tad will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _tadpos_plotting_command_args(self, request: Any, ignoreCount: bool) -> List[str]:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        plotter: str = request.get("plotter", "tadpos")
        command_args: List[str] = ["tad", "plotters", plotter]

        command_args.extend(self._common_plotting_command_args(request, ignoreCount))

        if plotter == "tadpos":
            command_args.extend(self._tadpos_plotting_command_args(request, ignoreCount))
        elif plotter == "madmax":
    ...
    async def start_plotting(self, request: Dict[str, Any]):
        ...
        plotter = request.get("plotter", "tadpos")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with TAD_ROOT: {os.environ['TAD_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"tad-blockchain version: {tad_full_version_str()}")
