...
from tad.data_layer.data_layer_wallet import DataLayerWallet
from tad.pools.pool_wallet import PoolWallet
from tad.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
from tad.protocols.protocol_message_types import ProtocolMessageTypes
from tad.protocols.wallet_protocol import CoinState
from tad.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
...
from tad.types.blockchain_format.coin import Coin, coin_as_list
...
from tad.util.errors import KeychainIsLocked
from tad.util.ints import uint8, uint32, uint64, uint16
...
from tad.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from tad.wallet.cat_wallet.cat_wallet import CATWallet
...
from tad.wallet.did_wallet.did_wallet_puzzles import match_did_puzzle, program_to_metadata
from tad.wallet.nft_wallet import nft_puzzles
from tad.wallet.nft_wallet.nft_info import NFTInfo, NFTCoinInfo
from tad.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from tad.wallet.nft_wallet.nft_wallet import NFTWallet
from tad.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from tad.wallet.notification_store import Notification
from tad.wallet.outer_puzzles import AssetType
from tad.wallet.puzzle_drivers import PuzzleInfo, Solver
from tad.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from tad.wallet.trade_record import TradeRecord
from tad.wallet.trading.offer import Offer
from tad.wallet.transaction_record import TransactionRecord
from tad.wallet.uncurried_puzzle import uncurry_puzzle
from tad.wallet.util.address_type import AddressType, is_valid_address
...
class WalletRpcApi:
    ...
    async def get_latest_coin_spend(
        self, peer: Optional[WSTadConnection], coin_id: bytes32, latest: bool = True
    ...
    async def get_offer_summary(self, request) -> EndpointResult:
        ...
        from tad.util.bech32m import bech32_decode, convertbits
        from tad.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    async def check_offer_validity(self, request) -> EndpointResult:
        offer_hex: str = request["offer"]
        offer = Offer.from_bech32(offer_hex)
        peer: Optional[WSTadConnection] = self.service.get_full_node_peer()
        if peer is None:
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "tad")
        ...
        key = None
        if asset_id is not None and asset_id != "tad":
    ...
    async def did_get_info(self, request) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.TAD.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        tad_coin_list = request.get("tad_coins", None)
        tad_coins = None
        if tad_coin_list:
            tad_coins = set([Coin.from_json_dict(tad_coin) for tad_coin in tad_coin_list])
        tad_change_target = request.get("tad_change_target", None)
        if tad_change_target is not None:
            if tad_change_target[:2] == "tad":
                tad_change_ph = decode_puzzle_hash(tad_change_target)
            else:
                tad_change_ph = bytes32(hexstr_to_bytes(tad_change_target))
        else:
            tad_change_ph = None
        ...
        if mint_from_did:
            ...
            sb = await nft_wallet.mint_from_tad(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
