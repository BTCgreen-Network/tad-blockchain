...
from tad.rpc.util import wrap_http_handler
from tad.server.outbound_message import NodeType
from tad.server.server import TadServer, ssl_context_for_client, ssl_context_for_server
...
from tad.util.config import str2bool
from tad.util.ints import uint16
from tad.util.json_util import dict_to_json_str
from tad.util.network import WebServer
from tad.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> TadServer:
        ...
        # a property so as to be read only which allows TadServer to satisfy
        # Optional[TadServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        tad.rpc.rpc_server.default_get_connections()
        """
        ...

    async def on_connect(self, peer: WSTadConnection) -> None:
...
def default_get_connections(server: TadServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
