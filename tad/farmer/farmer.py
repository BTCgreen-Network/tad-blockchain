...
from tad.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from tad.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from tad.util.errors import KeychainProxyConnectionFailure
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'tad keys generate' or open the UI."
        try:
        ...
        if "tad_target_address" not in self.config:
            self.config = config["farmer"]
        if "tad_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "tad_target_address" not in self.config or "tad_target_address" not in self.pool_config:
            log.debug("tad_target_address missing in the config")
            return False
