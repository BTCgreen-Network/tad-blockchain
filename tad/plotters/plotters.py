...
from tad.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from tad.plotters.tadpos import get_tadpos_install_info, plot_tad
from tad.plotters.madmax import get_madmax_install_info, plot_madmax
...
def build_parser(subparsers, root_path, option_list, name, plotter_desc):
    ...
    for option in option_list:
        ...
        if option is Options.NUM_BUCKETS:
            u_default = 0 if name == "tadpos" else 256
            parser.add_argument(
                "-u",
                "--buckets",
                type=int,
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in TAD_ROOT.
    tad_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within TAD_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("tad plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, tad_plotter_options, "tadpos", "Create a plot with the default tad plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'tad plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    ...
    if args.plotter is None:
        plotters.print_help()
    elif args.plotter == "tadpos":
        plot_tad(args, tad_root_path)
    elif args.plotter == "madmax":
        plot_madmax(args, tad_root_path, root_path)
    elif args.plotter.startswith("bladebit"):
        plot_bladebit(args, tad_root_path, root_path)
    elif args.plotter == "version":
        show_plotters_version(tad_root_path)
...
def get_available_plotters(root_path) -> Dict[str, Any]:
    ...
    tadpos: Optional[Dict[str, Any]] = get_tadpos_install_info()
    ...
    if tadpos is not None:
        plotters["tadpos"] = tadpos
...
def show_plotters_version(root_path: Path):
    ...
    if "tadpos" in info and "version" in info["tadpos"]:
        print(f"tadpos: {info['tadpos']['version']}")
