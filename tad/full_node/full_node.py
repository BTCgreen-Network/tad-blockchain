...
from tad.util.memory_profiler import mem_profile_task
...
class FullNode:
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSTadConnection) -> None:
        """
    ...
    async def on_connect(self, connection: ws.WSTadConnection) -> None:
        """
    ...
    def on_disconnect(self, connection: ws.WSTadConnection) -> None:
    ...
    def remove_subscriptions(self, peer: ws.WSTadConnection) -> None:
    ...
    async def _sync(self) -> None:
        ...
        try:
            ...
            peers_with_peak: List[ws.WSTadConnection] = [
            ...
            weight_proof_peer: ws.WSTadConnection = random.choice(peers_with_peak)
            self.log.info(
    ...
    async def sync_from_fork_point(
        ...
        peers_with_peak: List[ws.WSTadConnection] = self.get_peers_with_peak(peak_hash)
        ...
        async def fetch_block_batches(
            batch_queue: asyncio.Queue[Optional[Tuple[ws.WSTadConnection, List[FullBlock]]]]
        ) -> None:
            start_height, end_height = 0, 0
            new_peers_with_peak: List[ws.WSTadConnection] = peers_with_peak[:]
            try:
        ...
        async def validate_block_batches(
            inner_batch_queue: asyncio.Queue[Optional[Tuple[ws.WSTadConnection, List[FullBlock]]]]
        ) -> None:
            advanced_peak: bool = False
            while True:
                res: Optional[Tuple[ws.WSTadConnection, List[FullBlock]]] = await inner_batch_queue.get()
                if res is None:
        ...
        batch_queue_input: asyncio.Queue[Optional[Tuple[ws.WSTadConnection, List[FullBlock]]]] = asyncio.Queue(
            maxsize=buffer_size
        )
    ...
    def get_peers_with_peak(self, peak_hash: bytes32) -> List[ws.WSTadConnection]:
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSTadConnection) -> None:
    ...
    async def request_compact_vdf(
        self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSTadConnection
    ) -> None:
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
    ...
    async def respond_compact_vdf(
        self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSTadConnection
    ) -> None:
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
