...
from tad.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
...
from tad.util.api_decorators import get_metadata
from tad.util.errors import Err, ProtocolError
from tad.util.ints import uint16
from tad.util.network import WebServer, is_in_network, is_localhost
from tad.util.ssl_check import verify_ssl_certs_and_keys
...
class TadServer:
    ...
    all_connections: Dict[bytes32, WSTadConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        tad_ca_crt_path, tad_ca_key_path = tad_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                tad_ca_crt_path, tad_ca_key_path, public_cert_path, public_key_path
            )
        ...
        if local_type in authenticated_server_types:
            ...
            public_cert_path, public_key_path = public_ssl_paths(root_path, config)
            ssl_context = ssl_context_for_server(
                tad_ca_crt_path, tad_ca_key_path, public_cert_path, public_key_path, log=log
            )
    ...
    async def start_server(self, prefer_ipv6: bool, on_connect: Callable = None):
        if self.webserver is not None:
            raise RuntimeError("TadServer already started")
    ...
    async def incoming_connection(self, request):
        ...
        try:
            connection = WSTadConnection.create(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    def connection_closed(self, connection: WSTadConnection, ban_time: int, closed_connection: bool = False):
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use tad's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.tadcoins.com/") as resp:
                    if resp.status == 200:
